...
async function main() {
  ...
  const courseDefs = [
    {
      ...
      desc: 'Annual mandatory GDPR training',
      type: CourseType.MANDATORY,
      duration: 4,
    },
    {
      title: 'Anti-Bribery & Corruption Policy',
      desc: 'Anti-bribery awareness training',
      type: CourseType.MANDATORY,
      duration: 3,
    },
    {
    ...
    },
    {
      ...
      desc: 'Company values and ethical conduct',
      type: CourseType.MANDATORY,
      duration: 3,
    },
    {
      title: 'Leadership Fundamentals',
      desc: 'Core leadership skills for managers',
      type: CourseType.OPTIONAL,
      duration: 8,
    },
    {
      title: 'Agile & Scrum Practices',
      desc: 'Agile methodology essentials',
      type: CourseType.OPTIONAL,
      duration: 6,
    },
    {
      title: 'Data Analysis with Excel',
      desc: 'Intermediate Excel for data tasks',
      type: CourseType.OPTIONAL,
      duration: 4,
    },
    {
      title: 'Effective Communication',
      desc: 'Business communication best practices',
      type: CourseType.OPTIONAL,
      duration: 3,
    },
    {
      title: 'Project Management Essentials',
      desc: 'PMP foundations and PMI methodology',
  ...
  for (const emp of employees) {
    ...
    for (const course of shuffled) {
      ...
      const enPayload: DeepPartial<Enrollment> = {
        ...
        completedAt,
        dueDate: new Date(
          now.getTime() + randomInt(30, 120) * 24 * 3600 * 1000,
        ),
        startedAt:
          status !== EnrollmentStatus.NOT_STARTED
  ...
  for (const emp of employees.slice(0, 35)) {
    for (let m = 0; m < 6; m++) {
      ...
      const existing = await attendanceRepo.findOne({
        where: { userId: emp.id, courseId, sessionDate },
      });
      if (!existing) {
  ...
  for (const emp of employees.slice(0, 35)) {
    for (const mDef of milestoneDefs.slice(0, randomInt(1, 4))) {
      const existing = await competencyRepo.findOne({
        where: { userId: emp.id, competencyCode: mDef.code },
      });
      if (!existing) {
        const achieved = Math.random() < 0.65;
        await competencyRepo.save(
          competencyRepo.create({
            userId: emp.id,
            competencyCode: mDef.code,
            competencyName: mDef.name,
            status: achieved
  ...
  const riskRules = [
    ...
    },
    {
      ...
      definition: {
        metric: 'overdueComplianceCourses',
        operator: 'gt',
        threshold: 0,
    ...
    },
    {
      ...
      severity: RuleSeverity.HIGH,
      definition: {
        metric: 'competencyAchievementRate',
        operator: 'lt',
        threshold: 50,
        scoreContribution: 30,
      },
    ...
    },
    {
      ...
      definition: {
        metric: 'milestone',
        milestoneCode: 'DATA_PRIVACY_CERT',
        status: 'not_achieved',
    ...
    },
    {
      ...
      definition: {
        metric: 'milestone',
        milestoneCode: 'SECURITY_AWARE_L1',
        status: 'not_achieved',
        scoreContribution: 35,
      },
    },
    {
      ...
      severity: RuleSeverity.MEDIUM,
      definition: {
        metric: 'expiringCertificationsCount',
        operator: 'gt',
        threshold: 0,
        scoreContribution: 15,
      },
    },
    {
      ...
      description:
        'Employee hired within 90 days has not completed onboarding courses',
      severity: RuleSeverity.HIGH,
      definition: {
    ...
    },
    {
      ...
      description:
        'Composite engagement score below 30 (weighted: completion + attendance + assessment)',
      severity: RuleSeverity.MEDIUM,
      definition: {
        composite: true,
        factors: [
    ...
    },
    {
      ...
      description:
        'Employee has had 3 or more interventions in the last 12 months',
      severity: RuleSeverity.HIGH,
      definition: {
        metric: 'interventionCount12Months',
        operator: 'gte',
        threshold: 3,
        scoreContribution: 30,
      },
  ...
  for (let i = 0; i < interventionDefs.length; i++) {
    ...
    const count = await interventionRepo.count({
      where: { userId: emp.id, type: def.type },
    });
    if (count === 0) {
      await interventionRepo.save(
        interventionRepo.create({
          ...
          preInterventionScore: def.postScore
            ? def.postScore - randomFloat(5, 20)
            : null,
          completedAt:
            def.status === InterventionStatus.COMPLETED
              ? randomDate(new Date('2025-06-01'), now)
              : null,
          outcomeNotes:
            def.status === InterventionStatus.COMPLETED
              ? 'Employee completed required training successfully.'
              : null,
        }),
      );
    }
